  ...
  module Core
    module ToolChecker
      ...
      def self.find_crystalline : String?
        ...
        local_bin = Env::ROOT_DIR.join("bin", "crystalline#{Env.exe_ext}")
        ...
        {% if flag?(:windows) %}
          ...
          candidates = [
            ...
            File.join(prog_files, "Lapis", "bin", "crystalline.exe"),
      ...
      def self.check_crystalline : ToolStatus
        if c_path = find_crystalline
          ver = get_crystalline_version(c_path) || "installed"
          return ToolStatus.new(
            name: "crystalline",
            installed: true,
            version: ver,
