...
# =============================================================================
# =============================================================================
# Lapis::Test and Declarative DSL Macros are provided by `src/libgodot/testing.cr`
...
test_2d "Node2D position, rotation, and scale" do
  if node.is_a?(Godot::Node2D)
    ...
    assert_approx_eq node.position.x, 120.0_f32
    assert_approx_eq node.position.y, 240.0_f32

    node.rotation = 1.5708_f32
    assert_approx_eq node.rotation, 1.5708_f32, 0.001

    node.scale = Godot::Vector2.new(2.0, 2.0)
    assert_approx_eq node.scale.x, 2.0_f32
    assert_approx_eq node.scale.y, 2.0_f32
  else
    n = Godot.create(Godot::Node2D)
    n.position = Godot::Vector2.new(50.0, 60.0)
    assert_approx_eq n.position.x, 50.0_f32
    n.destroy
  end
end
...
test_3d "Node3D position, rotation, and scale" do
  if node.is_a?(Godot::Node3D)
    ...
    assert_approx_eq node.position.x, 10.0_f32
    assert_approx_eq node.position.y, 20.0_f32
    assert_approx_eq node.position.z, 30.0_f32

    node.scale = Godot::Vector3.new(3.0, 3.0, 3.0)
    assert_approx_eq node.scale.x, 3.0_f32
  else
    n = Godot.create(Godot::Node3D)
    n.position = Godot::Vector3.new(1.0, 2.0, 3.0)
    assert_approx_eq n.position.x, 1.0_f32
    n.destroy
  end
end
...
test_3d "CharacterBody3D velocity and is_on_floor" do
  ...
  assert_approx_eq cb.velocity.y, -9.8_f32
  assert_approx_eq cb.velocity.z, 5.0_f32
  cb.destroy
end
...
test_nodes "Godot.create instantiates native engine nodes" do
  ...
  assert_not_nil n2d
  assert_false n2d.pointer.null?

  n3d = Godot.create(Godot::Node3D)
  assert_not_nil n3d
  assert_false n3d.pointer.null?
...
test_signals "SignalSpy records signal emissions" do
  target = PropertyTestTarget.new
  spy = SignalSpy.new(target, "test_event_fired")
...
test_gdscript "Instantiating GDScript scene and accessing controller" do
  ...
  assert_not_nil root
  assert_eq root.name, "InteropRoot"
  root.destroy
  scene.destroy
end
...
test_gdscript "Calling GDScript format_greeting returns formatted String" do
  ...
  assert_eq greeting, "Hello from GDScript, CrystalDeveloper!"
  root.destroy
  scene.destroy
end
...
test_gdscript "Calling GDScript spawn_node_for_crystal returns Node" do
  ...
  assert_eq spawned.not_nil!.name, "SpawnedByGDScript"
...
test_gdscript "Passing Crystal node into GDScript inspect_crystal_node" do
  ...
  assert_true result.starts_with?("OK:CrystalWorkerNode:"), "Expected OK:CrystalWorkerNode, got #{result}"
  root.destroy
  scene.destroy
end
...
test_gdscript "GDScript reparents Crystal node via reparent_node_from_crystal" do
  ...
  assert_eq crystal_sub.get_parent.not_nil!.name, "StaticChild"
  root.destroy
  scene.destroy
end
...
test_mesh "BoxMesh size configuration and assignment to MeshInstance3D" do
  ...
  assert_approx_eq box.get_size.x, 2.5_f32
  assert_approx_eq box.get_size.y, 3.5_f32
  assert_approx_eq box.get_size.z, 4.5_f32

  mi.set_mesh(box)
  ret_mesh = mi.get_mesh
  assert_not_nil ret_mesh
  assert_false ret_mesh.pointer.null?
  ret_mesh.destroy
  mi.destroy
  box.destroy
end
...
test_mesh "StandardMaterial3D creation and color assignment" do
  ...
  assert_approx_eq mat.get_albedo.r, 0.8_f32
  assert_approx_eq mat.get_albedo.g, 0.2_f32
  mat.destroy
end
...
test_mesh "Instantiating 3D mesh scene (test_mesh_3d.tscn)" do
  ...
  root = scene.instantiate
  assert_not_nil root
  assert_eq root.name, "TestMeshRoot3D"

  box_instance = root.get_node("BoxMeshInstance")
  assert_not_nil box_instance
...
test_physics "Instantiating test_area_2d.tscn and checking hierarchy" do
  ...
  root = scene.instantiate
  assert_not_nil root
  assert_eq root.name, "TestArea2D"

  col_shape = root.get_node("CollisionShape2D")
  assert_not_nil col_shape
...
test_physics "Area2D collision_layer and collision_mask validation" do
  ...
  assert_eq area.get_collision_layer, 3_i64
  assert_eq area.get_collision_mask, 3_i64
  assert_true area.is_monitoring
  assert_true area.is_monitorable
  area.destroy
  scene.destroy
end
...
test_physics "Instantiating test_area_3d.tscn and checking hierarchy" do
  ...
  root = scene.instantiate
  assert_not_nil root
  assert_eq root.name, "TestArea3D"
...
test_physics "Area3D collision_layer and collision_mask validation" do
  ...
  assert_eq area.get_collision_layer, 5_i64
  assert_eq area.get_collision_mask, 5_i64
  assert_true area.is_monitoring
  assert_true area.is_monitorable
  area.destroy
  scene.destroy
...
test_scenes "Loading and instantiating test_dummy_3d.tscn" do
  ...
  inst = scene.instantiate
  assert_not_nil inst
  assert_eq inst.name, "TestDummy3D"
  inst.destroy
  scene.destroy
end
...
test_scenes "Loading and instantiating test_stress_1000.tscn" do
  ...
  inst = scene.instantiate
  assert_not_nil inst
  assert_eq inst.name, "StressArena"
  assert_not_nil inst.get_node("SpawnOrigin2D")
  assert_not_nil inst.get_node("SpawnOrigin3D")
  inst.destroy
  scene.destroy
end
...
test_prop "Export Property Getters and Setters round-trip" do
  ...
  assert_approx_eq target.float32_val, 45.67_f32

  target.float64_val = 123.456789
  assert_approx_eq target.float64_val, 123.456789

  target.int32_val = 999
  assert_eq target.int32_val, 999

  target.int64_val = 55555555555_i64
  assert_eq target.int64_val, 55555555555_i64

  target.bool_val = false
  assert_false target.bool_val

  target.string_val = "Updated String"
  assert_eq target.string_val, "Updated String"

  target.vec2_val = Godot::Vector2.new(11.0, 22.0)
  assert_eq target.vec2_val.x, 11.0_f32

  target.vec3_val = Godot::Vector3.new(1.0, 2.0, 3.0)
  assert_eq target.vec3_val.z, 3.0_f32

  target.color_val = Godot::Color::BLUE
  assert_approx_eq target.color_val.b, 1.0_f32

  target.rect2_val = Godot::Rect2.new(5.0, 5.0, 50.0, 50.0)
  assert_eq target.rect2_val.size.x, 50.0_f32
  target.destroy
end

test_prop "Custom computed getter returns calculated value" do
  target = PropertyTestTarget.new
  assert_approx_eq target.health_percentage, 100.0_f32
  target.destroy
end
...
test_prop "Custom setter clamps input within bounds" do
  ...
  assert_approx_eq target.clamped_health, 100.0_f32

  target.clamped_health = -25.0_f32
  assert_approx_eq target.clamped_health, 0.0_f32

  target.clamped_health = 75.0_f32
  assert_approx_eq target.clamped_health, 75.0_f32
  target.destroy
end

test_prop "Custom setter triggers side-effects" do
  target = PropertyTestTarget.new
  assert_eq target.dirty_trigger, 0

  target.dirty_trigger = 3
  assert_eq target.dirty_trigger, 3

  target.dirty_trigger = 2
  assert_eq target.dirty_trigger, 5
  target.destroy
end

test_prop "Custom tool button setter acts as action trigger" do
  target = PropertyTestTarget.new
  assert_false target.tool_button_trigger

  target.tool_button_trigger = true
  assert_true target.tool_button_trigger
  target.destroy
end
...
test_prop "Property hints registered correctly in ClassDB" do
  ...
  assert_eq range_prop.not_nil!.hint, 1_u32
  assert_eq range_prop.not_nil!.hint_string, "0,100,5"

  enum_prop = props.find { |p| p.name == "enum_hint_prop" }
  assert_not_nil enum_prop
  assert_eq enum_prop.not_nil!.hint, 2_u32
  assert_eq enum_prop.not_nil!.hint_string, "Low,Medium,High"

  file_prop = props.find { |p| p.name == "file_hint_prop" }
  assert_not_nil file_prop
  assert_eq file_prop.not_nil!.hint, 13_u32

  multi_prop = props.find { |p| p.name == "multiline_hint_prop" }
  assert_not_nil multi_prop
  assert_eq multi_prop.not_nil!.hint, 18_u32

  color_prop = props.find { |p| p.name == "color_no_alpha_prop" }
  assert_not_nil color_prop
  assert_eq color_prop.not_nil!.hint, 21_u32

  tool_prop = props.find { |p| p.name == "tool_btn_prop" }
  assert_not_nil tool_prop
  assert_eq tool_prop.not_nil!.hint, 39_u32
  assert_eq tool_prop.not_nil!.variant_type, 25
  assert_eq tool_prop.not_nil!.usage, 4_u32
  assert_eq tool_prop.not_nil!.type_name, "Callable"
  assert_eq tool_prop.not_nil!.hint_string, "Click Here"

  method_tool_prop = props.find { |p| p.name == "test_method_action" }
  assert_not_nil method_tool_prop
  assert_eq method_tool_prop.not_nil!.hint, 39_u32
  assert_eq method_tool_prop.not_nil!.variant_type, 25
  assert_eq method_tool_prop.not_nil!.usage, 4_u32
  assert_eq method_tool_prop.not_nil!.type_name, "Callable"
  assert_eq method_tool_prop.not_nil!.hint_string, "Method Action,Play"

  # Verify Proc Tool Buttons registered in PropertyInfo
  ptr_tool_prop = props.find { |p| p.name == "proc_btn_ptr" }
  assert_not_nil ptr_tool_prop
  assert_eq ptr_tool_prop.not_nil!.hint, 39_u32
  assert_eq ptr_tool_prop.not_nil!.variant_type, 25
  assert_eq ptr_tool_prop.not_nil!.usage, 4_u32
  assert_eq ptr_tool_prop.not_nil!.hint_string, "Proc Method Pointer Button,Action"

  lambda_tool_prop = props.find { |p| p.name == "proc_btn_lambda" }
  assert_not_nil lambda_tool_prop
  assert_eq lambda_tool_prop.not_nil!.hint, 39_u32
  assert_eq lambda_tool_prop.not_nil!.variant_type, 25
  assert_eq lambda_tool_prop.not_nil!.usage, 4_u32
  assert_eq lambda_tool_prop.not_nil!.hint_string, "Proc Lambda Button"

  dyn_tool_prop = props.find { |p| p.name == "proc_btn_dyn" }
  assert_not_nil dyn_tool_prop
  assert_eq dyn_tool_prop.not_nil!.hint, 39_u32
  assert_eq dyn_tool_prop.not_nil!.variant_type, 25
  assert_eq dyn_tool_prop.not_nil!.usage, 4_u32
  assert_eq dyn_tool_prop.not_nil!.hint_string, "Proc Dynamic Button"

  target = PropertyTestTarget.new
  assert_false target.tool_btn_fired
  target._godot_call_tool_button("tool_btn_prop")
  assert_true target.tool_btn_fired
  ...
  assert_not_nil storage_prop
  assert_eq storage_prop.not_nil!.usage, 2_u32
end
...
test_prop "Inspector Grouping annotations registered" do
  ...
  assert_not_nil cat
  assert_eq cat.not_nil!.name, "Stats"
...
test_prop "Class-level Tool and Icon annotations registered" do
  ...
  assert_eq entry.not_nil!.icon_path, "res://addons/crystal_integration/crystal.gdextension"

  tool2d_entry = Godot::ClassRegistry.find("ToolTester2D")
  assert_not_nil tool2d_entry
  assert_true tool2d_entry.not_nil!.is_tool
end
...
test_prop "Signal registration and type-safe emission" do
  ...
  assert_not_nil sig
  assert_eq sig.not_nil!.args.size, 1
