...
test_audio_server "AudioServer bus management, decibel volume and mute toggles" do
  ...
  TestFramework.assert_true server.get_bus_count >= 1_i64
  TestFramework.assert_eq server.get_bus_name(0_i64), "Master"
  ...
  begin
    ...
    TestFramework.assert_approx_eq server.get_bus_volume_db(0_i64), -6.0_f32, 0.1

    server.set_bus_mute(0_i64, true)
    TestFramework.assert_true server.is_bus_mute(0_i64)

    server.set_bus_mute(0_i64, false)
    TestFramework.assert_false server.is_bus_mute(0_i64)
  ensure
...
test_audio_server "Dynamic audio bus creation, naming, Reverb effect routing, and teardown" do
  ...
  TestFramework.assert_eq server.get_bus_name(initial_buses), "DynamicTestBus"
  TestFramework.assert_eq server.get_bus_index("DynamicTestBus"), initial_buses

  # Add Reverb effect
  reverb = Godot.create(Godot::AudioEffectReverb)
  reverb.set_room_size(0.75_f64)
  reverb.set_damping(0.4_f64)
  TestFramework.assert_approx_eq reverb.get_room_size.to_f32, 0.75_f32

  append_at_end = -1_i64
  server.add_bus_effect(initial_buses, reverb, append_at_end)
  TestFramework.assert_eq server.get_bus_effect_count(initial_buses), 1_i64

  ret_fx = server.get_bus_effect(initial_buses, 0_i64)
  TestFramework.assert_not_nil ret_fx
  TestFramework.assert_false ret_fx.pointer.null?

  # Remove effect & bus
  server.remove_bus_effect(initial_buses, 0_i64)
  TestFramework.assert_eq server.get_bus_effect_count(initial_buses), 0_i64

  server.remove_bus(initial_buses)
  TestFramework.assert_eq server.get_bus_count, initial_buses
end
...
test_audio_server "AudioStreamPlayer2D & 3D spatial properties and bus assignments" do
  ...
  TestFramework.assert_eq player2d.get_bus, "Master"
  TestFramework.assert_approx_eq player2d.get_max_distance, 1200.0_f32
  TestFramework.assert_approx_eq player2d.get_pitch_scale, 1.25_f32
  TestFramework.assert_approx_eq player2d.get_volume_db, -3.0_f32
  player2d.destroy
  ...
  TestFramework.assert_approx_eq player3d.get_unit_size, 15.0_f32
  TestFramework.assert_approx_eq player3d.get_max_distance, 500.0_f32
  player3d.destroy
end
